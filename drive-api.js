/* ════════════════════════════════════════════════════════
   WIROG DRIVE API — Browser Google Drive Client
  ...
  var DriveAPI = {
    ...
    async init() {
      ...
      initPromise = (async function() {
        ...
        var saved = null;
        try { saved = JSON.parse(localStorage.getItem('wirog_drive_token')); } catch(e) {}
      ...
      return new Promise(function(resolve, reject) {
        DriveAPI.init().then(function() {
          ...
          tokenClient.callback = function(resp) {
            ...
            accessToken = resp.access_token;
            localStorage.setItem('wirog_drive_token', JSON.stringify({
    ...
    signOut() {
      if (accessToken) {
        google.accounts.oauth2.revoke(accessToken, function() {
          accessToken = null;
          localStorage.removeItem('wirog_drive_token');
