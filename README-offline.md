# Foromane Offline / Sync — Quick Test & Setup
...
window.registerForomaneSW().then(async r => { console.log(r); if (r.ok) await r.ensureBackgroundSync?.(); });
...
- Add Jest unit tests for `PricingEngine` and `ForomaneDB` wrappers; or
