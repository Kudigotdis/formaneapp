...
async function main() {
  ...
  const cfgPath = path.join(root, 'foromane-config.json');
  const tokenPath = path.join(root, 'drive-oauth-token.json');
  if (!fs.existsSync(cfgPath)) {
    console.error('foromane-config.json not found');
    process.exit(1);
  }
  ...
  if (!clientId || !clientSecret) {
    console.error('Missing client_id or client_secret in foromane-config.json');
    process.exit(1);
  }
  ...
  if (!usersFolderId) {
    console.error('users_folder_id not set in foromane-config.json');
    process.exit(1);
  }

  const fileName = `foromane_test_${Date.now()}.json`;
  const content = JSON.stringify({ test: 'foromane-test', createdAt: new Date().toISOString() }, null, 2);
