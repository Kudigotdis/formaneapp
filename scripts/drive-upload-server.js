 ...
 *
 * Requirements:
 *  - place `foromane-config.json` in project root (contains client_id/client_secret)
...
function loadConfig() {
  const cfgPath = path.join(PROJECT_ROOT, 'foromane-config.json');
  if (!fs.existsSync(cfgPath)) throw new Error('foromane-config.json not found in project root');
