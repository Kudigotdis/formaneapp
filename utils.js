...
/* ─── PUSH NOTIFICATIONS ─── */
var WIROG_VAPID_PUBLIC_KEY = 'BBAHkFzRPE3sVJUzqJpOmC-RmDDvFv8QCRIblGPqw_JhVKxSGoQHQ_w_WzBqy7jbFGi1JN8JMg1R3_n05HmPjVY'; // Demo key

function isNotificationEnabled() {
  return localStorage.getItem('wirog_notifications') === '1';
}
...
async function subscribeToPush() {
  try {
    ...
    var sub = await reg.pushManager.subscribe({
      ...
      applicationServerKey: urlBase64ToUint8Array(WIROG_VAPID_PUBLIC_KEY)
    });
    localStorage.setItem('wirog_notifications', '1');
    localStorage.setItem('wirog_push_subscription', JSON.stringify(sub.toJSON()));
...
async function unsubscribeFromPush() {
  try {
    ...
    localStorage.setItem('wirog_notifications', '0');
    localStorage.removeItem('wirog_push_subscription');
    var statusEl = document.getElementById('notif-status');
...
function showOnboarding() {
  if (localStorage.getItem('wirog_onboarding_done')) return;
  _onboardingStep = 0;
...
function closeOnboarding() {
  localStorage.setItem('wirog_onboarding_done', '1');
