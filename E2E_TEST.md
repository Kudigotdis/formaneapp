...
- If your app includes `sw-register.js` in build or you evaluate it from console, `registerWirogSW()` is the one to call.
...
3) Confirm the queued item exists in IndexedDB: DevTools → Application → IndexedDB → `wirog-sync` → `queue`.
...
- `sw-register.js` — registration helper (call `registerWirogSW()` from console)
