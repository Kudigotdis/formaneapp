...
class AdminData {
  ...
  refresh() {
    ...
    this.promoRequests = JSON.parse(localStorage.getItem('wirog_promo_requests') || '[]');
    this.paymentRequests = JSON.parse(localStorage.getItem('wirog_payment_requests') || '[]');
    this.artworkSubmissions = JSON.parse(localStorage.getItem('wirog_artwork_submissions') || '[]');
    ...
    this.admins = JSON.parse(localStorage.getItem('wirog_admins') || '["admin"]');
  ...
  addAdmin(userId) {
    if (!this.admins.includes(userId)) {
      ...
      localStorage.setItem('wirog_admins', JSON.stringify(this.admins));
  ...
  // === FACEBOOK SCHEDULE ===
  getFacebookSchedule() {
    return JSON.parse(localStorage.getItem('wirog_facebook_schedule') || '[]');
  }
  ...
  approvePromoRequest(id) {
    ...
    if (idx > -1) {
      ...
      localStorage.setItem('wirog_promo_requests', JSON.stringify(this.promoRequests));
      this.refresh();
      return true;
    }
    return false;
  ...
  approvePaymentRequest(id) {
    ...
    if (idx > -1) {
      ...
      localStorage.setItem('wirog_payment_requests', JSON.stringify(this.paymentRequests));
      this.refresh();
      return true;
    }
    return false;
